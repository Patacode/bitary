...
RSpec.describe Bitary::Decorator::SingleMethod::TruthyEnforcer do
  let(:fake_class) do
    Class.new do
      def execute(**kwargs)
        kwargs
  ...
  describe '::new' do
    it 'returns a new instance with given wrappee and method' do
      deco = Bitary::Decorator::SingleMethod::TruthyEnforcer.new(
        fake_class.new,
        :execute
      )

      expect(deco).to be_instance_of(
        Bitary::Decorator::SingleMethod::TruthyEnforcer
      )
    end

    it 'raises an ArgumentError if given method is not a Symbol' do
      expect do
        Bitary::Decorator::SingleMethod::TruthyEnforcer.new(
          fake_class.new,
          'invalid'
        )
  ...
  describe 'wrappee instance method chosen by decorator' do
    before(:example) do
      @run_deco = Bitary::Decorator::SingleMethod::TruthyEnforcer.new(
        fake_class.new,
        :run
      )
      @execute_deco = Bitary::Decorator::SingleMethod::TruthyEnforcer.new(
        fake_class.new,
        :execute
      )
    end
